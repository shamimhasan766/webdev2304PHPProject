...
$(document).ready(function() {
    ...
    $('.table').on('click', '.save-row', function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'edit_services.php', // PHP script to handle database update
            data: {
                id: id,
                values: editedValues
            },
...
$('.delete').on('click', function(){
    ...
    Swal.fire({
		title: "Are you sure?",
		text: "You Want TO Delete The Service Title : "+name,
		icon: "warning",
		...
		if (result.isConfirmed) {
			...
			window.location.href = 'delete_service.php?id='+id+'&name='+name;
