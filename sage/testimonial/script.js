...
$(document).ready(function() {
    ...
    $('.table').on('click', '.save-row', function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'edit_testimonial.php', // PHP script to handle database update
            data: {
                id: id,
                values: editedValues
            },
...
$('.delete').on('click', function(){
    ...
    Swal.fire({
		title: "Are you sure?",
		text: "You Want TO Delete The Testimonial From '"+name+"'",
		icon: "warning",
		...
		if (result.isConfirmed) {
			...
			window.location.href = 'testimonial_delete.php?id='+id+'&name='+name;
